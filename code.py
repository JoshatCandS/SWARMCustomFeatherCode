...
def tileParseLine(line):
  ...
  if line[0:3] == "$MT":
    if line[4] != "C":
        list0 = ''.join(str(e) for e in line)
        parse0 = list0[3:-3]
        displayLine(2,"# in queue: " + parse0)
  parse2 = line[:-3].split(',') 
  if parse2[0] == '$TD SENT':
      displayLine(0, 'Message Sent, can move now')
      pixels[0] = (0,0,16,0)
      pixels[1] = (0,0,16,0)
      pixels.write()  
  parse = line[:-3].split(' ')
  ...
  if parse[0] == '$RT':
    if 'RSSI' in parse[1]:
      if ',' in parse[1]:
        ...
        print(rtdata)
        displayLine(3, 'R:' + rtdata['RSSI'] + ' S:' + rtdata['SNR'] + ' F:' + rtdata['FDEV'])
      else:
        rssi = parse[1].split('=')
        displayLine(1, "Current RSSI: " + rssi[1])
        irssi = int(rssi[1])
        lastRSSI = irssi
...
def gpsInit():
  global sentQuery
  sentQuery = False
  displayLine(0, 'Finding Location...')
  pixels[0] = (16, 0, 0, 0)
  pixels[1] = (16, 0, 0, 0)
  pixels.write()
  ...
  if config['interval'] > 0:
      if time.time() > nextGPSTime:
          if lastGN is not None and lastDT is not None:
              ...
              h = h + b'*%02X\n'%cs
              tile.write(h)
              displayLine(0, 'Message Loaded!')
              pixels[0] = (0, 16, 0, 0)
              pixels[1] = (0, 16, 0, 0)
              pixels.write()
              tile.write(makeTileCmd("$MT C=U"))
...
def buttonPoll():
  ...
  switchC.update()
  if switchC.rose:
    displayLine(0, 'Sending new message...')
    pixels[0] = (0,16,0,0)
    pixels[1] = (0,16,0,0)
    pixels.write()
    nextGPSTime = time.time()
    tile.write(makeTileCmd("$DT @"))
    tile.write(makeTileCmd("$GN @"))
    writePreferences()
    gpspoll()
